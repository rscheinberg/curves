...
void add_circle( struct matrix *edges, double cx, double cy, double cz, double r, double step ) {
    for(double t = 0; t < 1; t += step) add_edge(edges, r * cos(2 * M_PI * t) + cx, r * sin(2 * M_PI * t) + cy, cz, r * cos(2 * M_PI * (t + step)) + cx, r * sin(2 * M_PI * (t + step)) + cy, cz);
}
...
void add_curve( struct matrix *edges, double x0, double y0, double x1, double y1, double x2, double y2, double x3, double y3, double step, int type ) {
    ...
    for(double t = 0; t < 1; t += step) {
        x = xcoefs->m[0][0] * t * t * t + xcoefs->m[1][0] * t * t + xcoefs->m[2][0] * t + xcoefs->m[3][0];
        y = ycoefs->m[0][0] * t * t * t + ycoefs->m[1][0] * t * t + ycoefs->m[2][0] * t + ycoefs->m[3][0];
        add_edge(edges, xp, yp, 0, x, y, 0);
        xp = x;
        yp = y;
    }
}
